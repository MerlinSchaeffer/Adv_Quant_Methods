...
::: {.content-box-blue}
**Research question of the day:** is there a **liberty–equality trade-off**? 
Do state-owned (socialist) economies reduce poverty — at the price of fewer civil liberties?
:::
...
available for **most countries since 1789** — and reverse it, so that **higher = more
state ownership**.
...
::: {.lead}
From "is there a relationship?" to <br> "**how much** does poverty change with state ownership?"
:::
...
At the state ownership $= 0$ (here the scale's midpoint), predicted poverty is **`r round(coef(mod)[1], 2)`%**.

With every unit more state ownership, average poverty is **`r round(coef(mod)[2], 2)` percentage points `r ifelse(coef(mod)[2] > 0, "higher", "lower")`**

$R^2 = `r round(summary(mod)$r.squared, 3)`$: state ownership accounts for almost **none** of the variance in poverty across the world.
:::
...
"More state ownership *increases* poverty by `r round(coef(mod)[2], 2)` percentage points."
...
"*Among countries* with one unit more state ownership, average poverty is `r round(coef(mod)[2], 2)` percentage points higher."
