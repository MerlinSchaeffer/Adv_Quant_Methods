...
::: {.backgrnote}
One part of today's lecture per goal. Our running question: **is there a freedom–equality trade-off?**
:::
...
::: {.content-box-blue}
**Research question of the day:** is there a **freedom–equality trade-off**? Do state-controlled (socialist) economies reduce poverty — at the price of less freedom?
:::
...
\node[draw=kured, thick, rounded corners=3pt, inner xsep=0.4cm, inner ysep=0.3cm]
  (S) at (0,0) {\textbf{State control of the economy}};
\node[draw=black!75, rounded corners=3pt, inner xsep=0.4cm, inner ysep=0.3cm]
  (F) at (11,0) {\textbf{Freedom} (equality \& liberty)};
\node[draw=black!75, rounded corners=3pt, inner xsep=0.4cm, inner ysep=0.3cm]
...
$\rightarrow$ The measurable core of the concept: **state ownership and control of the economy**.
...
::: {.left-column}
**Freedom**, measured as *equality before the law & individual liberty* (`v2xcl_rol`):
impartial administration, access to justice, property rights, freedom from torture and
political killings, freedom of religion and of movement — for men *and* women.

**State control of the economy**: our reversed `v2clstown` (see previous slide).
:::
...
(Dat_vdem <- vdem %>% # The data ship with the vdemdata package
   ...
   # Reverse, so that higher = MORE state control
...
::: {.content-box-green}
**Reading the scale:** it works like a **z-score** (today's topic!): 0 ≈ the average country-year since 1789; around **+2** ≈ heavily state-controlled (North Korea); around **−2** ≈ almost fully private (Switzerland).
:::
...
Dat_vdem %>%
  ...
  arrange(state_control) %>%
  # Keep the 8 lowest, the 8 highest, and Denmark
  filter(row_number() <= 8 | row_number() > n() - 8 |
           country == "Denmark") %>%
  ggplot(aes(y = state_control,
             x = reorder(country, state_control))) +
  geom_col(fill = "#901A1E") +
  labs(y = "State control of the economy", x = "",
...
ggplot(data = Dat_vdem %>%
         filter(country %in% c("Denmark", "Germany",
                ...
                year >= 1900),
       aes(y = state_control, x = year, color = country)) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = c("Denmark" = "#901A1E",
                                "Germany" = "#425570",
                                "German Democratic Republic" = "#b5892c")) +
  labs(y = "State control of the economy", x = "", color = "",
       caption = "Source: V-Dem") +
  theme_minimal(base_size = 16) +
...
ggplot(data = Dat_vdem %>%
         filter(country %in% c("Denmark", "Germany",
                               "German Democratic Republic"),
                year >= 1900),
       aes(y = equal_liberty, x = year, color = country)) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = c("Denmark" = "#901A1E",
                                "Germany" = "#425570",
                                "German Democratic Republic" = "#b5892c")) +
  labs(y = "Equality before the law\nand individual liberty",
...
### What do we see?
::: {.content-box-blue}
**Discuss:** socialist East Germany (1949–1990) in both plots — what happens to *state control* and to *freedom* while the GDR exists, and after 1990? What does this one case suggest about our research question?
:::

::: {.content-box-red .fragment}
The GDR ran a far more state-controlled economy **and** offered far less freedom than West Germany or Denmark — one historical case *previewing* edge 3 of our triangle. But one case is an anecdote; now we test the pattern across **all** countries.
:::
...
### Use the WB API
```{r wb-data}
(Dat_poverty <- wb_data("SI.POV.DDAY", # Extreme poverty: below $3.00 a day (2021 PPP)
...
## Inner join: poverty meets freedom & state control
...
::: {.content-box-green}
One row per country: its most recent poverty estimate, matched to freedom and state control **in that same year**. All three corners of our triangle in one tibble.
:::
...
::: {.lead}
Edge 1 of the triangle: state control and poverty — first as a picture, then as *one* number.
:::
...
ggplot(data = Dat,
       aes(y = poverty, x = state_control, label = country)) +
  geom_text(size = 3) +
  ...
  labs(y = "% below $3.00 a day (~ kr. 600 a month in DK)",
       x = "State control of the economy") +
...
(Dat <- Dat %>%
   mutate( # z-standardize both variables
     z_state_control = scale(state_control) %>% as.numeric(),
...
quad_plot <- Dat %>%
  mutate(
    quadrant = case_when(
      (z_state_control < 0 & z_poverty > 0) |
        (z_state_control > 0 & z_poverty < 0) ~ "opposite signs",
      TRUE                                    ~ "same signs"
    )) %>%
  ggplot(aes(y = z_poverty, x = z_state_control)) +
  ...
  labs(y = "Extreme poverty (z)", x = "State control of the economy (z)") +
  theme_minimal(base_size = 14) +
  guides(color = "none")
...
::: {.content-box-red .fragment}
Essentially **no linear association**: state-controlled economies do *not* have systematically less extreme poverty.
:::
:::
...
::: {.right-column}
<iframe src='2-exercise1.html' width='100%' height='620' frameborder='0' scrolling='auto'></iframe>
:::
...
::: {.lead}
From "is there a relationship?" to "**how much** does poverty differ with state control?"
:::
...
Then we could state *how much* poverty differs, per unit of state control.
...
mod <- lm(poverty ~ state_control, data = Dat)
...
::: {.push-right}
$\widehat{\text{Poverty}} = `r round(coef(mod)[1], 2)` + `r round(coef(mod)[2], 2)` \times \text{State control}$

At the scale's midpoint (state control $= 0$), predicted poverty is **`r round(coef(mod)[1], 2)`%**.

With every unit more state control, average poverty is **`r round(coef(mod)[2], 2)` percentage points `r ifelse(coef(mod)[2] > 0, "higher", "lower")`** — and $R^2 = `r round(summary(mod)$r.squared, 3)`$: state control accounts for almost **none** of the variance in poverty across the world.
:::
...
"More state control *changes* poverty by `r round(coef(mod)[2], 2)` percentage points."
...
"*Among countries* with one unit more state control, average poverty is `r round(coef(mod)[2], 2)` percentage points different."
...
::: {.right-column}
<iframe src='2-exercise2.html' width='100%' height='620' frameborder='0' scrolling='auto'></iframe>
:::

## The verdict: no freedom–equality trade-off
...
\draw[<->, kured, very thick] (F) -- node[below right=2pt] {{\small 2: $r < 0$ -- freer, less poor}} (P);
\draw[<->, kured, very thick] (S) -- node[above=2pt] {{\small 3: $r \ll 0$ -- strong freedom cost}} (F);
\end{tikzpicture}
```

::: {.content-box-green}
State-controlled economies offer **much less freedom** (edge 3) and **no less poverty** (edge 1) — while freer countries tend to have *less* poverty (edge 2). The premise of the trade-off does not hold in today's data.
:::
