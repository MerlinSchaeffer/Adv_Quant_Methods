...
::: {.notes}
Frame the day: our goal is not describing patterns but *explaining* them — and today is about avoiding **false** explanations. Walk the arc once: a real-world puzzle, then a simple but flawed analysis, then the theory that fixes it (potential outcomes), then the tool that draws it (DAGs).
:::
...
::: {.notes}
Say the paradox out loud, because it is genuinely counterintuitive: the *better* integrated report *more* discrimination, not less. Point at the forest plot — 280 estimates from 42 studies, so this is a robust pattern, not one odd survey. Today we test one proposed mechanism: awareness, via news consumption.
:::
...
::: {.notes}
The key idea: testing a hypothesis *always* implies a comparison — causality is a counterfactual, a "what if". Give them two minutes on Marx and Simmel, then collect: Marx compares capitalist against non-capitalist labour, Simmel urban against rural life. Every causal claim names two worlds, even when it doesn't say so.
:::
...
::: {.notes}
Ask first — give them two minutes before you click the reveal. Expect: social desirability, poor memory, lack of self-awareness. The line to land: people tell interesting stories about their lives, they do not record causal facts about themselves. So we need a comparison, not an introspection.
:::
...
::: {.notes}
Keep this short — it is scaffolding, not the point. Two things only: the outcome `dis_index` averages the six discrimination domains into one index, and the cause comes in two forms, total daily minutes and a "reads news" dummy at 15 minutes. Then move on; the payoff is the next slide.
:::
...
::: {.notes}
Ask them to interpret the scatter *before* you show the table — two minutes with a neighbour. The line is flat, if anything slightly negative, so visually it does not support our hypothesis. Then the regression: about −0.09, not significant. Let them draw the obvious conclusion that the hypothesis is wrong — and then ask whether this is a *fair* comparison. That question is the rest of the lecture.
:::
...
[**Open exercise 1 in a new tab ↗**](5-exercise1.html){target="_blank"}

<div class="ku-timer" data-min="20"></div>
:::

::: {.right-column}
<iframe src='5-exercise1.html' width='100%' height='620' frameborder='0' scrolling='yes' style="border:1px solid #ddd; border-radius:6px;"></iframe>
:::
...
::: {.notes}
Ferda makes it concrete: 4 with news, 2 without, so her personal causal effect is +2. Now the hard part — in reality we see only *one* of those two rows; the other is gone forever. That is the fundamental problem of causal inference, and it is exactly why we cannot fix this by asking people to imagine the other path.
:::
...
::: {.notes}
We give up on personal effects and aim for the average instead. Conceptually it compares two whole worlds — everyone reads the news, nobody does — and neither is ever observed. But the average *is* estimable under the right conditions, and what those conditions are is essentially the rest of this course.
:::
...
::: {.notes}
Since we cannot see Ferda's missing future, we substitute a different person — that is what every observational comparison does. The result is −1, a terrible estimate of either person's true effect. Give them two minutes on "why is this so wrong?"; the answer you are steering to is that they were different people **to begin with**.
:::
...
::: {.notes}
Head off the panic: this looks like heavy algebra, but all we did was add and subtract the *same* term, Ferda's $Y_0$. That splits the observed −1 into the true effect, +2, plus something else worth −3. Name that something else: selection bias — Tuki's baseline is simply higher, so even with no news at all she would report more.
:::
...
::: {.notes}
Same logic as Ferda and Tuki, now for whole groups — the maths is identical, only the labels changed. Left of the equals sign is what we compute; right is what we want plus what contaminates it. Point hard at the unobserved term: it is the one we can never check directly. Then use the green box to let them brainstorm why news readers might differ at baseline — education, age, language, citizenship.
:::
...
::: {.notes}
The move here: we can never compare the *unobserved* baseline, but we can compare observed characteristics — and if the groups differ on those, they almost certainly differ on $Y_0$ too. Walk the code briefly, then read the table: readers are around 44 versus 37 and more often German citizens. State the conclusion flatly — the groups are imbalanced, so the naïve comparison is definitely biased. And stress: these are only the differences we can *see*.
:::
...
::: {.notes}
A DAG maps your causal *assumptions*, not your data. The puppet metaphor works well: grab a node, shake it, and the movement travels along the arrows. Shake $C$ and both $D$ and $Y$ move together — that is the spurious correlation. Shake $D$ and $Y$ does not move at all, because there is no arrow between them. A confounder is just a variable sitting on such a backdoor path.
:::
...
::: {.notes}
Now make it concrete with our own case. German citizenship plausibly drives *both* whether people follow German news *and* how much discrimination they actually encounter. Trace the backdoor path with your finger: reads news ← German citizen → discrimination. That path on its own could produce the negative correlation we saw — no causal arrow required. And this is exactly the imbalance the balance table found.
:::
...
::: {.notes}
Close the loop: they now know *why* this flat line cannot be read causally — confounders sort people into and out of reading the news, so the baselines differ. Ice cream and shark attacks is the throwaway example if anyone still resists. Then bridge to next week: if the problem is that the groups differ to begin with, the fix is to *make* them comparable — which is what randomisation does.
:::
...
[**Open exercise 2 in a new tab ↗**](5-exercise2.html){target="_blank"}

<div class="ku-timer" data-min="20"></div>
:::

::: {.right-column}
<iframe src='5-exercise2.html' width='100%' height='620' frameborder='0' scrolling='yes' style="border:1px solid #ddd; border-radius:6px;"></iframe>
:::
...
::: {.notes}
Run through this fast — they are tired by now. If only one sentence survives the day, make it this one: an observed difference in means is a causal effect **plus** selection bias, and everything we do from here on is about forcing that second term to zero.
:::
