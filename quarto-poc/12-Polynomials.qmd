...
### The picture
```{r lin-plot, echo = FALSE, out.width='48%', fig.height = 3.8, fig.width = 7.5}
ggplot(Dat, aes(y = LifeExpect, x = HealthExp)) +
  ...
  ggrepel::geom_text_repel(data = Dat_lab, aes(label = country, colour = country),
                           size = 3.4, fontface = "bold", seed = 1, min.segment.length = 0) +
...
ggplot(Dat, aes(y = LifeExpect, x = HealthExp)) +
  ...
  labs(y = "Life expectancy", x = "Health spending per person ($, PPP) - log2 scale") +
  theme_minimal(base_size = 13) + guides(colour = "none")
```
