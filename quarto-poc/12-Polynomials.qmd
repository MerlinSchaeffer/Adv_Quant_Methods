...
### The picture
```{r lin-plot, echo = FALSE, out.width='62%', fig.height = 4.4, fig.width = 8}
ggplot(Dat, aes(y = LifeExpect, x = HealthExp)) +
  geom_point(alpha = 0.15, colour = "grey45") +
  geom_smooth(method = "lm", se = FALSE, colour = "#901A1E", linewidth = 1.2) +
  labs(y = "Life expectancy at birth", x = "Health spending per person ($, PPP)") +
  theme_minimal(base_size = 14)
```

::: {.content-box-red}
**The line misses the shape.** It under-predicts in the middle and over-predicts at the extremes — a textbook sign the **linearity assumption is violated**.
:::
...
::: {.notes}
Show the picture first — the straight red line slicing through an obvious curve is more persuasive than any statistic. Name the tell-tale pattern: the line sits below the cloud in the middle and above it at both ends, which is exactly what a mis-specified linear fit looks like. The $R^2$ of about 0.45 is not terrible, which is the trap — a mediocre linear fit can still look respectable in a table while being the wrong shape.
:::
...
ggplot(Dat, aes(y = LifeExpect, x = HealthExp)) +
  geom_point(alpha = 0.15, colour = "grey45") +
  geom_smooth(se = FALSE, colour = "#901A1E", linewidth = 1.2) +   # LOESS is the default
...
ggplot(Dat, aes(y = LifeExpect, x = HealthExp)) +
  ...
  geom_smooth(method = "lm", formula = y ~ poly(x, 2, raw = TRUE),
              se = FALSE, colour = "#901A1E", linewidth = 1.2) +
  labs(y = "Life expectancy", x = "Health spending per person ($, PPP)") +
  theme_minimal(base_size = 13)
```
...
## The log₂ idea: counting doublings

::: {.push-left}
The **base-2 logarithm** answers *"how many times do I double?"*

$$\log_2(2) = 1, \quad \log_2(4) = 2, \quad \log_2(8) = 3$$

Each **+1 on the log₂ scale = a doubling** of the original value.

::: {.content-box-green}
Perfect for health spending: going \$500 → \$1,000 is *one doubling*, and so is \$4,000 → \$8,000. If each doubling buys the **same** extra years, a curve on the raw scale becomes a **straight line** on the log scale.
:::
:::
...
tibble(x = 2^(0:8)) %>%
  ...
  labs(x = "Original value", y = expression(log[2](x))) +
  theme_minimal(base_size = 13)
```
:::

::: {.notes}
Log base 2 is the most teachable log because the unit is concrete: one step is one doubling. Do a couple out loud — 2, 4, 8 — until they feel the pattern. Then the key insight for modelling: if life expectancy rises by a constant amount per *doubling* of spending rather than per dollar, then plotting against log-spending turns the curve into a straight line, and a straight line is something OLS handles perfectly. That is why the transform works.
:::
...
ggplot(Dat, aes(y = LifeExpect, x = HealthExp)) +
  ...
  scale_x_continuous(trans = log2_trans(),
                     breaks = c(25, 100, 400, 1600, 6400)) +
  labs(y = "Life expectancy", x = "Health spending per person ($, PPP) — log₂ scale") +
  theme_minimal(base_size = 13)
```

::: {.backgrnote .center}
On a log₂ x-axis, the curve straightens into a line.
:::
:::
