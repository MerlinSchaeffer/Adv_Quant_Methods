...
```{r include = FALSE}
coef <- coef(zols)["articleTreat_1"] %>% round(., 3) %>% unname()
se <- vcov(zols) %>% diag()
se <- se["articleTreat_1"] %>% sqrt() %>% round(., 3) %>% unname()
```
...
```{r include = FALSE}
coef <- coef(zols)["articleTreat_2"] %>% round(., 3) %>% unname()
se <- vcov(zols) %>% diag()
se <- se["articleTreat_2"] %>% sqrt() %>% round(., 3) %>% unname()
```
