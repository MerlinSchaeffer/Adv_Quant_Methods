...
cat(gsub("&amp;nbsp;", "",
         as.character(modelsummary(list("50-person sample" = mod_se),
...
[3]{.alert} **If $H_0$ were true,** samples of 50 would scatter around 0 like this ($t$-distribution, df = `r ht$df`).
:::

::: {.fragment fragment-index=3}
[4]{.alert} **Test interval:** 95% of those chance estimates land within $0 \pm `r round(qt(0.975, ht$df), 2)` \times \hat\sigma$.
:::

::: {.fragment fragment-index=4}
[5]{.alert} **Our estimate** $\hat\beta = `r round(ht$estimate, 3)`$ is $t = `r round(ht$statistic, 2)`$ standard errors from 0 — **inside** the interval.
:::

::: {.fragment fragment-index=5}
[6]{.alert} `r round(100 * (1 - ht$p.value))`% of chance estimates are *closer* to 0 than ours …
:::

::: {.fragment fragment-index=6}
[7]{.alert} … and the other $p = `r round(ht$p.value, 2)`$ are at least as far out. Not surprising → we **can't reject** $H_0$.
:::
:::
...
**All `r format(nrow(ESS), big.mark = ",")` Danes:** a *smaller* estimate, but far **outside** a much narrower interval → $p = `r format(signif(ht_full$p.value, 2))`$, reject $H_0$.
