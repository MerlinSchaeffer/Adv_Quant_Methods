...
subtitle: "You practice: data preparation, simulating random assignment, and balance tables. When you're done, assist some of the others …"
...
> - `load("APAD.RData")` fails? The file must sit **in your project folder** — download it from Absalon and check the *Files* pane.
> - Strange results? *Session → Restart R*, then rerun your script from the top.
...
4. **Write one or two sentences** in your R script (as a `# comment`) with your conclusion
   from the balance table. Then compare:

`r hide("Model answer")`
*Because the candy was assigned by a (simulated) coin flip, the groups look similar on
gender and physical appearance — randomization balanced them without us controlling for
anything. The remaining small differences are chance: with a finite sample, randomization
balances groups only* **on average** *(and larger groups balance better than small ones).*
`r unhide()`
