...
.left-column[
.content-box-green[
If we continuously drew random samples and estimated the interval $\hat{\beta} \pm \text{Critical value} * \hat{\sigma}$, how many of these would contain $\beta$?
]
]
.right-column[
...
.push-left[
<br>
Thanks, that also clarifies quite a bit of the R output we haad ignored until now. But there is two more, we have not discussed yet: 
...
- $p$-value: The area outside $H_{0}:\beta=0 \pm t$
...
#First subst the data and add the coordinates to make it shade to y = 0
shade <- rbind(c(-teh*se,-teh*se), subset(data, betas > -teh*se & betas < teh*se), c(teh*se, 0))
...
- Randomness prohibits systematic bias!
  + Random samples are therefore representative.
