...
mod1 <- lm(trstplc ~ eduyrs, data = ESS)
coef <- coef(mod1)["eduyrs"] %>% round(., 3) %>% as.scalar()
```

1. Disregarding the weights for a moment: How much is a one-year increase in education associated with an average change in trust in the police? The answer is `r fitb(coef, width = "5")`
...
```{r}
# Run the regression and make a regression table.
mod2 <- lm(trstplc ~ gndr, data = ESS)
screenreg(mod2, digits = 3)
```
`r unhide()`
...
screenreg(list(mod1, mod2, mod1_weighted, mod2_weighted), 
          include.ci = FALSE, digits = 3,
          custom.model.names = c("Unweighted", "Unweighted", 
                                 "Weighted", "Weighted"))
```
`r unhide()`
