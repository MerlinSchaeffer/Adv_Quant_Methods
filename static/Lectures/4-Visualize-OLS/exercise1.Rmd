...
2. Make a scatterplot with citizenship rights as outcome on the Y-axis and the years since independence as predictor on the X-axis. Make sure to add a regression line with 95% confidence interval to it.
`r hide("R solution -> dont' peak to early ;) !")`
...
3. Run an OLS regression with citizenship rights as outcome and the years since independence as predictor. Is the association statistically significant? `r mcq(c(answer = "Yes, the SE is < 1/2 the coefficient", "No"))` 
`r hide("R solution -> dont' peak to early ;) !")`
```{r out.width='100%', fig.height = 4, fig.width = 7.5, results = FALSE, echo = TRUE}
...
4. Re-run the model using `lm()` and check whether Estonia is an outlier and whether the linearity assumption is strongly violated. `r mcq(c("Yes, Estonia is an outlier with a large Cook's D", "Yes, the association is clearly not linear", answer = "No, Estonia is no outlier and the association is roughly linear"))` 
`r hide("R solution -> dont' peak to early ;) !")`
```{r out.width='100%', fig.height = 4, fig.width = 7.5, results = FALSE, echo = TRUE}
...
4. How does the Danish colonial legacy compare to that of other countries in terms of extreme poverty? `r mcq(c("Not significantly different", "Worse than in the reference (Belgium)", answer = "Significantly better than in the reference (Belgium)"))`
`r hide("R solution -> dont' peak to early ;) !")`
```{r out.width='100%', fig.height = 4, fig.width = 7.5, results = FALSE, echo = TRUE}
