...
knitr::include_graphics('https://sociology.fas.harvard.edu/sites/hwpi.harvard.edu/files/styles/profile_full/public/sociology/files/legeiwe_headshot_for_crop_1_0.jpg?m=1689020156&itok=ESF3upCN')
```
...
knitr::include_graphics('https://upload.wikimedia.org/wikipedia/commons/thumb/c/cb/John_Snow_memorial_and_pub.jpg/1024px-John_Snow_memorial_and_pub.jpg')
```
...
The absolute correlation between intervention $I$ and predictor of interest $D$ is 1, making them practically the same thing.
<br>
<br>
...
The absolute correlation between intervention $Z$ and predictor of interest $D$ is smaller than 1, which induces some random variation into $D$.
...
1. *First stage*: Instrument has causal effect on $D_{i}$.

2. *Randomization*: Instrument is randomly assigned (RCT or natural experiment).

3. *Exclusion restriction*: Instrument affects outcome $Y_{i}$ **_only_** through its effect on $D_{i}$.
]]
...
$$\underbrace{\kappa_{\text{Win lottery}\rightarrow\text{Income}}}_{1624\text{ Dollar}} = \underbrace{\kappa_{\text{Win lottery}\rightarrow\text{Moving}}}_{0.4766} \times \underbrace{\kappa_{\text{Moving}\rightarrow\text{Income}}}_{?}$$
...
$$\begin{equation}\begin{split} & \underbrace{\kappa_{\text{Win lottery}\rightarrow\text{Income}}}_{1624\text{ Dollar}} = \underbrace{\kappa_{\text{Win lottery}\rightarrow\text{Moving}}}_{0.4766} \times \underbrace{\kappa_{\text{Moving}\rightarrow\text{Income}}}_{?} \\ \\ & \frac{\kappa_{\text{Win lottery}\rightarrow\text{Income}}}{\kappa_{\text{Win lottery}\rightarrow\text{Moving}}} = \underbrace{\kappa_{\text{Moving}\rightarrow\text{Income}}}_{?} \end{split}\end{equation}$$
]
---
layout: false
