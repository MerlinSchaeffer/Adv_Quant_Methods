...
ESS <- read_dta("../../../assets/Legewie_ESS_02.dta") %>%
  ...
  # Sample restriction: Keep only cases from Portugal!        
...
```{r}
plotdata_PT <- ols_pt %>%
  tidy() %>% # Convert regression output to tidy data frame
  filter(term != "(Intercept)") %>% # Remove the intercept row from results
  mutate(
    term = case_when(
      term == "treatAfter the attack" ~ "After the attack"))  # Rename treatment variable for plot

# Create plot using ggplot2
ggplot(data = plotdata_PT, 
       # Set main aesthetics: effect size on y, term on x
       aes(y = estimate, x = term, 
           ymin = conf.low, ymax = conf.high)) +  # Add confidence intervals
  geom_hline(yintercept = 0, color = "red",  # Add horizontal line at y=0 for reference
             lty = "dashed") + 
  geom_pointrange() + # Plot point estimates with confidence intervals
  coord_flip() + # Flip coordinates to make horizontal plot
  theme_minimal() + # Use minimal theme for clean look
  # Add title and axis labels
  labs(title = "Causal effect of Bali 2002 terror attack",  
       x = "", 
       y = "Difference in xenophobia compared to before the attack
...
ESS <- read_dta("../../../assets/Legewie_ESS_02.dta") %>%
  mutate(
    ...
    # Weights
    pspwght = pweight * dweight) %>%
  # Sample restriction: Keep only cases from Portugal or Sweden!        
...
4. Adapt the data preparation code under "# Sample restriction: ..." so that the sample also entails Swedes in addition to Portuguese respondents. To estimate the causal effect of the Bali 2002 terror attack among Swedes use `data = ESS %>% filter(cntry == "SE")` in your `lm_robust()` code. The causal effect of the Bali 2002 terror attack among Swedes is `r fitb(coefSE, width = "5")` and `r mcq(c("Significant", "Statistically significant", answer = "Not statistically significant"))`.
`r hide("R solution -> dont' peak to early ;) !")`
```{r ref.label = "olsSE"}
```
`r unhide()`
...
plotdata <- bind_rows(plotdata_SE, plotdata_PT, .id = "cntry") %>% # Combine Swedish and Portuguese data
 mutate(
   cntry = case_when(
     cntry == 1 ~ "Sweden", # Label country code 1 as Sweden
     cntry == 2 ~ "Portugal")) # Label country code 2 as Portugal

ggplot(data = plotdata, 
      aes(y = estimate, x = reorder(cntry, estimate), # Create plot, order countries by effect size
          ymin = conf.low, ymax = conf.high)) + # Add confidence intervals
 geom_hline(yintercept = 0, color = "red", lty = "dashed") + # Add reference line at zero
 geom_pointrange() + # Plot point estimates with confidence intervals
 coord_flip() + # Flip coordinates to make horizontal plot
 theme_minimal() + # Use minimal theme for clean look
 labs(title = "Causal effect of Bali 2002 terror attack", # Add title and axis labels
   x = "", 
   y = "Difference in xenophobia compared to before the attack
   (in standard deviations)")
```
