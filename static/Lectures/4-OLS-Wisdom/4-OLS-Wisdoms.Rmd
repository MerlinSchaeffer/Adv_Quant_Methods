...
---
# LPM versus logistic regression
...
LPMs violate the linearity assumption and can thus predict $\hat{\text{Pr}}(y_i = 1|x_{i}) < 0$, or $\hat{\text{Pr}}(y_i = 1|x_{i}) >1$. This is controvertial!

But we accept that problem, because the alternative (i.e. logistic regression) comes with many other problems `r Citep(myBib, "breen_interpreting_2018")`.
]
