...
ggplot(data = Dat %>% filter(!is.na(colonizer)), # Make coord system for data,
  ...
  labs(y = "Nr of countries colonized", x = "") + # Axis labels,
  theme_minimal()
```
]
...
- **Z-standardized residuals:** Distance from regression line; Examples: 
  + Nr. 62 Haiti (poverty rate `r Dat$poverty[Dat$country == "Haiti"]`%).
  + Nr. 32 Congo, Dem. Rep. (poverty rate `r Dat$poverty[Dat$country == "Congo, Dem. Rep."]`%).
  + Nr. 69 Ireland (poverty rate `r Dat$poverty[Dat$country == "Ireland"]`%).
...
```{r ref.label = "outlier", out.width='75%', fig.height = 4, fig.width = 5, results = FALSE, echo = FALSE}
```
]
...
```{r ref.label = "linearity", out.width='75%', fig.height = 4, fig.width = 5, results = FALSE, echo = FALSE}
```
]
...
```{r ref.label = "linearity2", out.width='75%', fig.height = 4, fig.width = 5, results = FALSE, echo = FALSE}
```
]
...
.right-column[
```{r indep-world2, out.width='100%', fig.height = 4, fig.width = 6, results = FALSE, echo = FALSE}
ggplot(data = Dat %>% filter(!is.na(colonizer) & !is.na(years_indep)), # Make coord system for data,
       aes(y = poverty, # Y- and X-axis of plot,
           x = years_indep)) +
  geom_smooth(method = "lm") +
...
ggplot(data = Dat_bi, aes(y = poverty, x = colonizer_bi)) +
  ...
  geom_smooth(method = "lm") +
  scale_x_continuous(breaks = c(0, 1), labels = c("Belgium", "Britain")) +
...
Dat <- Dat %>%
  mutate(
    colonizer = colonizer %>% fct_relevel("Belgium", "France", "Britain"))
```
```{r ols_2, eval = FALSE}
...
```{r ref.label = "categorical", out.width='75%', fig.height = 4.5, fig.width = 6.5, results = FALSE, echo = FALSE}
```
]
.push-right[
...
- Average poverty in Belgian-colonized countries: `r round(coef(ols_2)[1], 2)`% 
  + (When France = Britiain = 0)

- French-colonized countries compared to Belgian: `r round(coef(ols_2)[2], 2)` percentage points lower poverty on average
...
- British-colonized countries compared to Belgian: `r round(coef(ols_2)[3], 2)` percentage points lower poverty on average
  + Average poverty in British-colonized countries: `r round(coef(ols_2)[1], 2)`% + `r round(coef(ols_2)[3], 2)`% = `r round(coef(ols_2)[1], 2) + round(coef(ols_2)[3], 2)`%
]]]]
