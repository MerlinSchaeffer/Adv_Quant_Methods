...
LPMs violate the linearity assumption and can thus predict $\hat{\text{Pr}}(y_i = 1|x_{i}) < 0$, or $\hat{\text{Pr}}(y_i = 1|x_{i}) >1$. This is controversial!
]
...
ggplot(data = plotdata, aes(y = estimate, 
  ...
  labs(
    ...
    y = "Difference in extreme poverty compared to countries colonized by Britain") +
  theme_minimal()
```
]
