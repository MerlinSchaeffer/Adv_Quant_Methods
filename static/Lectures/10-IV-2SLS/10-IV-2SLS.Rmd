...
2. Two more IV applications:
  1. Does reading improve Kids' school performance?
...
---
# Instrument vs control variables
...
An instrument variable $Z$ induces unconfounded random variation into $D$. 

By using IV, we only keep that part of $D$ that **_correlates_** with $Z$.
]]
...
A confounder $C$ induces omitted variable/selection bias into $D$. 

By controlling for $C$, we only keep that part of $D$ that **_does not correlate_** with $C$.
]]
...
1. The instrument does not induce random variation but omitted variable bias (OVB) into $D$. Remember, a backdoor path is any path from $D$ to $Y$ that starts with an arrow into $D$.

2. By using IV, we weigh up it's OVB by $\frac{1}{\phi}$!
]
...
.content-box-green[
Exclusion assumption: Do you think the weather affects school performance *only* through it's effect on how much kids read?
]]
