...
1. The APAX data also contains a variable `antidiscr_law` on support for anti-discrimination policy: "How good or bad is it for a country to have a law against ethnic discrimination in the work place?". The answer categories are: (1) Very bad, (2) Bad, (3) So/so, (4) Good, (5) Very good. Use the APAX survey experiment. Estimate the causal effect of reading an article on support for anti-discrimination policy. Estimate z-standardized effects,
...
```
3. What is the causal effect of reading news about integration in society on supporting equal treatment policies? The causal effect is: `r fitb(coef, width = "5")`. What is the estimated standard error of this $\hat{\beta}$? `r fitb(se, width = "5")`. *What do these two statistics suggest about a causal effect?*

4. Make a coeffient plot to visualize these results.
`r hide("R solution -> dont' peak to early ;) !")`
```{r}
(plotdata <- zols %>%
