...
- The difference in average xenophobia between people living in Denmark or Bulgarian is larger among better educated people.
...
modelsummary(list("Xeno" = ols_mult_1, "Xeno" = ols_inter),
             gof_map = c("nobs", "r.squared"), stars = TRUE, output = "gt")
```
]]
...
.push-left[
Interaction terms, $\beta_3(X_{1i}\times X_{2i})$, have two interpretations!
```{r inter3, echo = FALSE}
# Report results in a regression table
...
knitr::include_graphics('https://upload.wikimedia.org/wikipedia/commons/thumb/3/32/Marx_-_Engels_-_Lenin_-_Stalin.svg/984px-Marx_-_Engels_-_Lenin_-_Stalin.svg.png')
```
]
...
# 1. Download equality before the law & individual freedom data for all countries since 1972,
...
# 3. Join VDem and Worldbank data
...
ggplot(data = Dat, 
       aes(y = poverty, x = equal_liberty, label = country)) +
  geom_smooth(method = "lm") +
  geom_text() +
  geom_label_repel(data = Dat %>% filter(country == "Denmark"), 
                   show.legend = FALSE, color = "#901A1E",
...
Dat <- Dat %>% mutate( # Mean-center predictors
  ...
  mc_equal_liberty = equal_liberty - mean(equal_liberty, na.rm = TRUE))

# Multiple OLS
ols_add <- lm_robust(poverty ~ mc_socialist + mc_equal_liberty, data = Dat)
...
library(rockchalk)
plotPlane(ols_add, "mc_equal_liberty", "mc_socialist", pch=16, alty=1,alwd=1, theta=50, phi=10)
```
]]
...
# Multiple OLS
ols_inter <- lm_robust(poverty ~ mc_socialist*mc_equal_liberty, data = Dat)
...
# The 3-D plot
library(rockchalk)
plotPlane(ols_inter, "mc_equal_liberty", "mc_socialist", pch=16, alty=1,alwd=1, theta=50, phi=10)
```
]

.push-right[
```{r fig.height = 5, fig.width = 5, echo = FALSE, out.width='80%'}
plotPlane(ols_inter, "mc_equal_liberty", "mc_socialist", pch=16, alty=1,alwd=1, theta=35, phi=20)
```
]]
...
$\beta_{\text{Citizen Rights}}|\text{Socialism}=0$: At average levels of socialism, a unit increase in citizenship rights is significantly related to a `r coef(ols_inter)["mc_equal_liberty"]` percentage point decline in poverty.

$$\beta_{\text{Socialism} \times \text{Citizen Rights}}$$
1) With every unit more citizenship rights, socialism gets `r coef(ols_inter)["mc_socialist:mc_equal_liberty"]` percentage points worse at being related to declines in poverty.

2) With every additional year of socialism, citizenship rights are `r coef(ols_inter)["mc_socialist:mc_equal_liberty"]` percentage points less associated with declines in poverty.
