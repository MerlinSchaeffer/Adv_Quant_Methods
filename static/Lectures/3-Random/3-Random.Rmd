...
.push-left[
Download the "ESS9e03_1.sav" data from Absalon. Place them into the folder for this course (where your R-project is located).
]
.push-right[
```{r ESS, echo = FALSE}
...
knitr::include_graphics('https://republicpolicy.com/wp-content/uploads/2024/07/world-population-day.jpg')
```
]
...
.push-right[
```{r, echo = FALSE, out.width='80%', fig.align='center'}
knitr::include_graphics('https://cdn.dribbble.com/users/1731126/screenshots/7225519/media/f7015f6a25ed60946661e1ec9435d083.gif')
```
...
.center[
.content-box-green[
What is the percent <br> of "Yes"-votes (disregarding the weights)?
]
]
]
...
```{r, echo = FALSE, out.width='60%', fig.align='center'}
knitr::include_graphics('https://mediaplaynews.b-cdn.net/wp-content/uploads/2020/09/PlayStation-Gamer-Girts-Ragelis-Shutterstock-WEB.jpg')
```
```{r, echo = FALSE, out.width='100%', fig.align='center'}
...
.push-left[
<br>
OK, great: Randomness prohibits systematic biases.

It thus enables us to draw representative samples. 

If randomization has partly failed, we use post-stratification weights to counterbalance.
...
**Solution**: *Random sampling* prohibits systematic biases, *weighting* alleviates partly-failed randomization.
...
```
.backgrnote[.center[Random sampling ensures validity (i.e. unbiasedness), but it results in some unreliability (i.e. sampling error). <br> The question is: How unreliable are the data?]]
]
---
# Sampling error $\sigma$ .font60[(aka sampling variability)]

.push-left[
- **Let's assume, the Danish ESS with its `r nrow(ESS)` respondents was our (unobservable super) population of interest.** .backgrnote[Thus no weights in this example ;)]

- That would mean, we can *calculate* the true OLS regression line (in blue): An increase of education by 1-year, goes along with an increase in political efficacy of $\beta = `r coef(lm(psppsgva ~ eduyrs, data = ESS))["eduyrs"]`$.
]
...
my_animation <- ggplot(data = Samples, aes(y = psppsgva, x = eduyrs)) +
  geom_jitter(color = "#901A1E", alpha = 1/3, width = 0.1, height = 0.1) +
...
animate(my_animation, nframes = 400, fps = 20)
```
]
...
.font80[
```{r}
# Critical value for 95%, with df = 50 - 2:
(crit_val <- qt(p = 0.975, df = 48))

# Assuming the t-distribution,
# 95% of the estimates of beta 
# will lie in the interval
...
.push-left[
<br>
I can't sample the whole (super) population.

But then I certainly can't draw thousands of $n=50$ samples either.
]
...
.push-right[.font90[
> Sure, more educated people may read more news, but that just means they're more aware of all the corruption. 

> Their belief in political efficacy isn't any different from anyone else's! 

> Your data showing otherwise is nothing more than a fluke - a statistical accident. Don't read too much into it.
...
.push-left[.font70[
1. $t$-statistic: $t=\frac{\hat{\beta}}{\hat{\sigma}}$
  + Tells us how many standard errors $(\hat{\sigma})$ our estimated $\hat{\beta}$ is away from 0.
  + "How far is our estimate from what we'd expect if there was no effect?"


2. $p$-value: Area under the sampling distribution in both tails outside $0 \pm t$
  + $P(|\hat{\beta}| \geq \text{observed } \hat{\beta}\text{ } | \text{ } \beta = 0)$
  + "If there truly was no effect, how likely would we be to see a result this extreme or more extreme?"
  + A small $p$-value (typically < 0.05) suggests: observed result is unlikely under the null hypothesis.
  + This doesn't prove $H_{1}$, but it provides evidence against $H_{0}$.
...
.left-column[
```{r, echo = FALSE, out.width='100%'}
knitr::include_graphics('https://www.chalmers.se/_next/image/?url=https%3A%2F%2Fcms.www.chalmers.se%2FMedia%2Fmb3hi4kn%2Fcoding-7-1920.jpg%3Fwidth%3D1920%26height%3D1080%26v%3D1d98d6204a2cd70%26quality%3D60%26format%3Dwebp&w=3840&q=90')
```
...
.font90[
- Randomness prevents systematic bias, ensuring representative samples unless sample size is very small.
- When random sampling isn't feasible, post-stratification weights can counterbalance bias.
- Random sampling introduces sampling error, which affects reliability.
- These errors follow a normal or t-distribution, depending on the sample size.
- We can estimate sampling error from our data: $\hat{\sigma}$.


- Using properties of normal and t-distributions, we can:
- Estimate 95% confidence intervals to express uncertainty.
- Conduct hypothesis tests to assess the likelihood of chance findings.


- Hypothesis tests evaluate the null hypothesis $(H_0)$, typically the opposite of what we're trying to demonstrate.
- They calculate the probability ($p$-value) of observing our data if $H_0$ were true.
- The $t$-value represents how many standard errors $\hat{\sigma}$ our estimate (e.g., $\hat{\beta}$) is from the null hypothesis (usually 0).
- The $p$-value is the area under the sampling distribution outside $\pm t$. 
...
1. `geom_smooth(method = "lm")`
  + Adds Ordinary Least Squares (OLS) regression lines to scatter plots
  + Part of the ggplot2 package for data visualization


2. Weighted analyses
  + Most R functions include a weights argument
  + Allows for incorporation of survey weights or other weighting schemes


3. `modelsummary()`
  + Versatile function for creating customized regression tables
  + Offers various options for table design and output format
