...
4. Also estimate the bivariate un-adjusted effect and make a coefficient plot of the two results; don't report the intercept. Use `bind_rows(..., .id = "adjusted")` to combine both into one tibble. What do we learn by comparing the two estimates? `r mcq(c("Bot much, both effects are significant.", answer = "The bivariate effect is a bit too large; there was some selection into treatment that introduced omitted variable bias", "The result from multiple OLS is more uncertain and hence less trustworthy"))`.
...
plotdata <- bind_rows(  # Combine the results from the two models 
  tidy(ols_pt_biv),     # Extract results from the bivariate model
  tidy(ols_pt_adj),     # Extract results from the adjusted model
  .id = "adjusted") %>%  # Create a variable named "adjusted" to distinguish between the models
  mutate(               # Create a new variable with more descriptive labels
    adjusted = case_when( 
      adjusted == 1 ~ "Bivariate",  # Label the bivariate model as "Bivariate"
      adjusted == 2 ~ "Adjusted")) %>%  # Label the adjusted model as "Adjusted"
  filter(term == "treatAfter the attack")  # Keep only the row with the treatment effect

ggplot(data = plotdata, aes(y = estimate, x = adjusted, ymin = conf.low, ymax = conf.high)) +
  geom_hline(yintercept = 0, color = "red", lty = "dashed") + # Add a horizontal line at zero 
  geom_pointrange() +                                         # Create a point range plot (estimates with confidence intervals)
  coord_flip() +                                              # Flip the x and y axes
  theme_minimal() +                                          # Use a minimal theme
  labs(title = "Causal effect of Bali 2002 terror attack among the Portuguese",  # Add a title
...
5. The adjusted causal effect of the Bali 2002 terror attack among Finns is `r fitb(coefFI, width = "3")` and `r mcq(c("Significant", "Statistically significant", answer = "Not statistically significant"))`.
`r hide("R solution -> dont' peak to early ;) !")`
```{r ref.label = "olsSE"}
```
`r unhide()`