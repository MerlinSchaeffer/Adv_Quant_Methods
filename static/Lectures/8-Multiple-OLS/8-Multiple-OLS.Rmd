...
**Reduced Form:** $\beta_{Z \rightarrow Y} = E[Y_{i}|Z_{i}=1]-E[Y_{i}|Z_{i}=0]; \text{call this }\rho.$
...
**Reduced Form:** $\rho = 0.114$ .backgrnote[According to `r Citet(myBib, "sherman_specific_1984")`.]
...
toydat <- toydat %>% 
  mutate( # Day of interview: Age + random.
    ...
    bali = case_when( # Bali: After mean int_day (survey period).
...
modelsummary(list("Resid. Xeno" = ols_e), 
            stars = TRUE,
            gof_map = c("nobs", "r.squared"), 
            output = "kableExtra")
```
...
.center[Time of measurement helps to decide whether a variable is a confounder or a mechanism/mediator.<br> But generally, you can only rely on sound logical reasoning. .alert[Don't control for mechanisms/mediators.]]
