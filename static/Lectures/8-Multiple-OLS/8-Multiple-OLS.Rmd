...
subtitle: "Adjusting randomized interventions <br><br> for *observed* confounders"
...
---
# Pre- versus post-treatment controls

.center.alert[Time of measurement helps to decide whether a variable is a confounder or a mechanism/mediator.<br> But generally, you can only rely on sound logical reasoning!]
...
- *Potential confounders*: Variables that are **_causally prior to the treatment_**.
]]

.push-left[.content-box-red[
.center[**Post-treatment**]
```{tikz, DAG_mediator, echo = FALSE, out.width = '40%'}
\usetikzlibrary{shapes,decorations,arrows,calc,arrows.meta,fit,positioning,quotes}
\tikzset{
  -Latex,auto,node distance =1 cm and 1 cm,semithick,
  state/.style ={ellipse, draw, minimum width = 0.7 cm},
...
\node[state] (3) [gray, double, above = of 2] {$M$};
...
\end{tikzpicture}
```
- *Potential mechanisms/mediators*: Variables that are potentially **_a result of the treatment_**.
]]
...
.content-box-green[
1. Download "Sesame-variables.doc" from Absalon. Remember that our outcome was "postlet", "encouraged" was the randomized intervention, and "watched" was the treatment of interest. 

2. Identify variables that should ideally be balanced and if not should be controlled for as pre-treatment confounders.

3. identify variables that we should definitly not control for because they are post-treatment mechanisms, or even alternative measures of the outcome we are interested in.
]
