...
subtitle: "8. Multiple OLS regression: Adjusting randomized interventions for *observed* confounders"
...
\begin{tikzpicture}
  \node[bag] {Randomized \\ $I$ntervention of, or $E$xposure to \\ $D$}
    child {node[bag] {Researcher\\randomized $I$}
...
1. **Today's 1st research question**: Terrorist attacks and xenophobia re-visited.
...
4. **Today's 2nd research question**: Zero-tolerance for domestic violence re-visited.
...
.content-box-green[
What kind of bias does this imply for our causal terrorist attack estimate? Is it our estimate most likely too large or too small?
]
]
...
toydat <- toydat %>% 
  mutate( # Day of interview: Age + random.
    ...
    bali = case_when( # Bali: After mean survey period.
      int_day < mean(int_day) ~ 0,
      int_day >= mean(int_day) ~ 1))

ols <- lm_robust(bali ~ age, data = toydat)
modelsummary(list("Day of Int." = ols), stars = TRUE,
...
modelsummary(list("Biv. OLS" = ols_bi, "Mult. OLS" = ols_mult), stars = TRUE,
             gof_map = c("nobs", "r.squared"))
```
...
1. We .alert[know] that one part of the difference is the causal effect $\beta_{\text{sample}} = 0.3 + \underbrace{\sigma_{\text{sample}}}_{\text{sampling error}} = `r coef(ols_mult)["bali"]`$.

.content-box-green[
- Why do we .alert[know] that in this specific case?

- What else is different between those with Bali = 0 compared to those with Bali = 1?
]]
...
modelsummary(list("Xeno" = ols_age), stars = TRUE,
             gof_map = c("nobs", "r.squared"))
```
]]
...
`r Citet(myBib, "frisch_partial_1933")` proved that the way multiple OLS achieves this is mathematically equivalent to three steps.
...
modelsummary(list("Resid. Xeno" = ols_e), stars = TRUE,
             gof_map = c("nobs", "r.squared"))
```
]]
.push-right[
...
# Regression table
modelsummary(list("First stage" = first_stg), stars = TRUE,
             gof_map = c("nobs", "r.squared"))
```
]]
.panel[.panel-name[R code]
...
---
layout: true
# What if .font70[the instrument wasn't random?]
...
# Regression table
modelsummary(list("Z" = ols_zc, "D" = ols_dc), stars = TRUE, 
...
- **Ethnicity may also introduce OVB to $\lambda$**, if Asian suspects have different repeat domestic violence rates. Unfortunately, the accessible data do not entail $Y$, so we cannot check and correct for this.
]]
...
# Regression table
modelsummary(list("D" = ols_dc), stars = TRUE, 
             statistic = NULL, gof_map = c("nobs", "r.squared"))
```
]
...
.content-box-green[
1. Download the "Sesame-variables.doc" file from Absalon. Recall that our outcome was "postlet", the randomized intervention was "encouraged", and the treatment of interest was "watched".

2. Identify variables that should ideally be balanced, and if they are not, should be controlled for as pre-treatment confounders.

3. Identify variables that we should definitely not control for because they are post-treatment mechanisms or even alternative measures of the outcome we are interested in.
]
