...
1. If we want to improve comparisons to get as close as possible to the causal effect $D \rightarrow Y$, then we need to control for all observed confounders and for no mediators!

2. If we want to test how much a mediator "explains" the causal effect $D \rightarrow Y$
...
1. The tool of multiple OLS and its Frisch-Waugh logic can be used for many purposes. Tyopically we use it for the four purposes outlined today.

2. We can improve balance for (natural) experimental designs.

3. We can improve comparisons by adjusting for confounders, ideally with a smart summary variable that blocks all backdoor paths.

4. We can improve comparisons by ruling out alternatives.

5. Confounder: A variable that lies on a backdoor path from treatment to outcome (i.e. a path that starts with an arrow into the treatment).

6. Mediators/mechanisms: A variable that is part of the causal channel via which the treatment affects the outcome of interest.
---
# References
