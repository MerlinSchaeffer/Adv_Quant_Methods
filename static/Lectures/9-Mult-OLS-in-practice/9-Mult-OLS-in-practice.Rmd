...
# Multiple OLS
ols_mult <- lm_robust(
  poverty ~ socialist + citizen_rights, data = Dat)
# Regression table
screenreg(ols_mult, include.ci = FALSE, digits = 3)
```
]
.push-right[
...
.push-left[
**Step 1**: Generate fictional data of $x1$ with informative values. .alert[Set controls to informative constant value (e.g. the average)!]
.font90[
```{r}
(fict_dat <- tibble(
  socialist = 0:100,
  citizen_rights = mean(Dat$citizen_rights, na.rm = TRUE)))
```
]]
.push-right[
...
ggplot(data = fict_dat, aes(y = fit, x = socialist)) +
  ...
  labs(
    ...
    (At average values of citizenship rights)",
    # Write Greek beta into axis title.
