...
plotPlane(ols_1, "citizen_rights", "socialist", theta = 35, phi = 20)
```
...
# Step 1 <br> <br> .font80[Get a first impression using LOESS <br> (locally estimated scatterplot smoothing)]

---
class: clear
# LOESS .font60[(locally estimated scatterplot smoothing)]
...
ggplot(data = Dat, aes(y = LifeExpect, x = HealthExp)) +  
  ...
  geom_dl(data = Dat %>% filter(country == "Switzerland" | country == "Denmark"),  
          ...
          aes(label = country, color = country), method = list(dl.combine("last.points"))) +  
  # Add a LOESS to the plot to show the overall trend, colored black.
...
ggplot(data = fict_dat, aes(y = fit, x = HealthExp)) +
  ...
  geom_text(
    data = Dat %>%  # Use original 'Dat' data for labeling
      filter(date == 2020 &  # Filter for the year 2022 and specific countries
