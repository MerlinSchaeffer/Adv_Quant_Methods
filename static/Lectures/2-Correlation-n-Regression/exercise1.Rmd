...
1. Make a folder for this course and set up an RStudio project. then start a new R-Script and copy the lines of R code from my slides that generate 'Dat'. That is, download Freedom House and WB data, join them, and construct our socialism index.
...
# Make sure to run the two lines of code below only once, to delete them afterwards, 
# and to un-comment "democracyData" in the pacman::p_load() above afterwards.
remotes::install_github("xmarquez/democracyData")
pacman::p_load(democracyData) # download democracy data used in the scholarly literature.
...
2. Make a scatterplot with poverty as outcome on the Y-axis and citizenship rights as predictor on the X-axis.
`r hide("R solution -> dont' peak to early ;) !")`
...
3. Estimate the correlation of poverty and citizenship rights (rounded to two digits): `r fitb(cor_povcitiz, width = "5")`
`r hide("R solution -> dont' peak to early ;) !")`
```{r}
Dat %>% 
  select(poverty, citizen_rights) %>% 
...
`r mcq(c(answer = "No, where people have rights there is also less poverty", "Yes, limited freedom is the price to pay for equality", "The result is inconclusive"))`
