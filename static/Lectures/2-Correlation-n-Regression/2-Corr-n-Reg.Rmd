...
.left-column[
We focus on: **Equality before the law & individual freedom:** impartial public administration, transparent laws with predictable enforcement, access to justice for men/women, property rights for men/women, freedom from torture, freedom from political killings, from forced labor for
men/women, freedom of religion, freedom of foreign movement, and freedom of domestic movement for men/women.
]
.right-column[
.panelset[
...
.panel[.panel-name[Denmark & Germany since 1789]
```{r ref.label = "citiz-DK", out.width='100%', fig.height = 4, fig.width = 8, results = FALSE, echo = FALSE}
```
]

.panel[.panel-name[Code] 
```{r citiz-DK, fig.show = 'hide', echo = TRUE}
ggplot(data = Dat_vdem %>% filter(country == "Denmark" | country == "Germany"), # Make coordinate system for data from Denmark,
       aes(y = equal_liberty, x = year, # Y- and X-axis of plot,
           color = country)) +
  geom_line() + # plot data as is in a bar chart,
  labs(y = "Index of equality before the law \n and individual liberty", x = "", cap = "Source: V-Dem", color = "") + # Axis labels,
  theme_minimal() + # Simple background layout,
  theme(axis.text.x = element_text(angle = 60, hjust = 1), legend.position="bottom") # Write country names in a 60 degree angle.
```
]

.panel[.panel-name[Across the world '24]
```{r ref.label = "citiz-world", out.width='100%', fig.height = 8, fig.width = 14, results = FALSE, echo = FALSE}
```
]

.panel[.panel-name[Code] 
```{r citiz-world, fig.show = 'hide', echo = TRUE}
ggplot(data = Dat_vdem %>% filter(year == 2024), # Make coordinate system for data from 2022,
       aes(y = equal_liberty, # Y- and X-axis of plot,
           x = reorder(country, equal_liberty))) +
  geom_bar(stat = "identity") + # plot data as is in a bar chart,
  labs(y = "Index of equality before the law \n and individual liberty in 2024", x = "", cap = "Source: V-Dem") + # Axis labels,
  theme_minimal() + # Simple background layout,
...
```{r, echo = FALSE, out.width='90%'}
knitr::include_graphics('https://media.geeksforgeeks.org/wp-content/uploads/20230216170349/What-is-an-API.png')
```
...
.push-left[
```{r}
(Dat_vdem <- Dat_vdem %>% select(-country)) # Drop the country variable
```
]
...
ggplot(data = Dat, aes(y = poverty, x = equal_liberty)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  labs(y = "% population with less than US$2.15 a day\n (like living in Denmark with DKK420 per month)", 
       x = "Index of equality before the law and indivial liverty") +
  theme_minimal() +
  guides(color = "none")
```
...
ggplot(data = Dat, aes(y = equal_liberty, x = socialist)) +
  ...
  labs(x = "Our socialism index", 
       y = "Index of equality before the law and indivial liverty") +
  theme_minimal() +
  guides(color = "none")
```
]
