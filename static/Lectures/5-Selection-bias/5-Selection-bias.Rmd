...
- Our goal: not just describing patterns, but explaining them.

- Today is about avoiding false explanations.
...
1. Start with a real-world puzzle.

2. Show a simple but flawed analysis.

3. Introduce the theory to fix it (Potential Outcomes).

4. Introduce the tool to visualize it (DAGs).
...
- Why do subjective evaluations of the situation deteriorate?

- Objective context may not have improve much, but did it really become worse?
...
- The central puzzle: The Integration Paradox.
  + It's counterintuitive: more integration -> more reported discrimination. 

- Meta-analysis on right confirms this pattern: Indicator of integration -> more reported discrimination
  + Estimates + 95Ci from +40 studies
---
class: inverse
...
- Ask them to interpret the regression result.

- Narrow the focus: Awareness. 
  + More integrated people consume more news, making them more aware of discrimination. 

- Frame this as the hypothesis we will test today.
...
- Ask class: "Why is just asking people a bad way to test this?"
  + Give. them 2 minutes to discuss
  + (Social desirability, poor memory, lack of self-awareness).

Click 2: Main point: self-reports are not credible causal evidence.
  + People tell interesting stories about their life, they do not record causal facts.
  + Need better, more objective methods.
...
???
- Key idea: Testing hypotheses always implies a comparison.
---
class: clear
# Hypothesis $\rightarrow$ Comparison
...
???
Causality = counterfactual ("what if").
...
???
Translate H_1 into a mathematical comparison:
- We compare the average outcome of news-readers vs. non-readers.
...
???
Ask Students to reinforce counterfactuals with classic examples:
- Marx: capitalism vs. non-capitalism.
- Simmel: urban vs. rural life.
- Give them two inutes to discuss.
---
# Preparation
...
???
Go through setup. Briefly explain:
- Content of survey data and its questions.
- Data preparation and coding of variables:
  + `discrimination` is the outcome: Average across dimensions
  + `news consumption` is the cause: binary and total minutes
...
Let's do the simplest possible comparison.
...
???
Ask class to interpret scatter plot (2 minutes with neighbor)
- Regression line is flat / slightly negative.
- Visually, does not support our hypothesis.
...
???
Ask class to interpret OLS table
- Focus on categorical news_yn: -0.093.
- News-readers report slightly *less* discrimination.
- Not statistically significant ($\beta < 1.96 \times \text{SE}$).
- Conclusion: Hypothesis seems wrong. 

But is this analysis valid?
---
...
???
Quick recap of Stats I.

- Graph shows sampling distribution under the null (H_0: effect is 0).
- Our estimate (-0.093) is the dotted line.
- Not outside the red $H_0$-"rejection" region $(H_0 \pm 1.96 \times \text{SE})$
- Outside orange region $\rightarrow$ p-value > 0.05, so we fail to reject the null.
...
???
The key statistical theory: Potential Outcomes Framework
- Key idea: For anyone, there are two potential states (e.g., news reader vs. no news reader).
- The causal effect is the difference between these two states.
- Fundamental problem of causal inference: We only ever see one state.

---
# Potential outcomes .font60[Ferda's *personal causal effect*]
...
Concrete with a hypothetical person: Ferda.

- $Y_1$: Outcome if she gets treatment (reads news).

- $Y_0$: Outcome if she doesn't.

- Personal causal effect: $Y_1−Y_0=+2$.

Punchline: In reality, we only observe $Y_1$ or $Y_0$ for Ferda.
---
...
Goal: the Average Causal Effect (ACE) of all individual effects.

Conceptually: compare a world where everyone gets treatment to a world where no one does.

Estimable under certain conditions!
---
layout: false
...
???
We only observe one path per observation. The other is forever unknown.

**This is why we need methods that don't rely on asking people to _imagine_ the other path!**
---
layout: true
...
Can we just compare two different people?

Introduce Ferda (reads news, we see $Y_1=4$) and Tuki (no news, we see $Y_0=5$).

Grey numbers are unobserved counterfactuals.

The "true" effects are +2 for Ferda, 0 for Tuki.
---
...
???
Do the naïve comparison: Ferda's outcome - Tuki's outcome = -1.

Terrible estimate of either person's true effect!

"Why is this comparison so wrong?" (give 2 minutes to discuss)

"They are different people *to begin* with."
---
...
???
Looks complex, but we just add something that is overall 0

Allows us to see: True Causal Effect (+2)

...plus something else, called Selection Bias (-3)
---
...
???
Define Selection Bias: The groups were different *to begin* with.

$Y_0,Ferda \neq Y_0,Tuki$

Tuki's baseline discrimination is higher than Ferda's. This pre-existing difference contaminates our comparison.
---
layout: false
...
Blue box: What we can see in the data. The average of each group.

Red box: The unobserved counterfactuals. The missing pieces of the puzzle.

Our naïve comparison only uses the blue box info!
---
...
???
Now we generalize from Ferda and Tuki to populations; logic remains the same!

Left side: The Average Causal Effect (what we want).

Right side: The naïve difference in means (what we calculate).
---
# Why? Selection bias!
...
???
Same mathematical addiotion of term that equals 0!

Observed Difference = Causal Effect + *Selection Bias*

Highlight the formula for selection bias. 

Emphasize that one term is **unobserved**.
--
...
???
Let them brainstorm reasons why news-readers might be different.

(Education, age, language, citizenship ...)
...
???
How to check for *observed* bias? A balance test.

We can't compare unobserved things, but we can compare observed characteristics.

Explain logic of R code

Balance table:
- They differ on age, citizenship, immigrant generation!
- Conclusion: The groups are imbalanced. Our naïve comparison is definitely biased
---
class: inverse middle center
# Directed Acyclical Graphs (DAGs)

???
We know bias exists. Now, a visual tool to think about it.
...
???
DAG: a visual map of our causal assumptions.

Backdoor Path: a non-causal path connecting Treatment (D) and Outcome (Y).

A Confounder is a variable on a backdoor path. It's the source of the bias.

Think of DAG as a puppet that you shake at a variable. 

Shaking transfrs along the edges
---
...
If you shake C, D and Y covary.

If you shake D, Y does not move!

non-causal (spurious) correlation between D and Y!
---
...
???
Confounder (C): German Citizenship

Citizenship -> affects news reading (D)

Citizenship -> affects discrimination experiences (Y)

Backdoor path: Read news <- German citizen -> Discrimination.

This path could explain our negative correlation.
---
layout: false
...
???
Now we know WHY we can't interpret this as causal.

Selection Bias: The groups differ on confounders.
