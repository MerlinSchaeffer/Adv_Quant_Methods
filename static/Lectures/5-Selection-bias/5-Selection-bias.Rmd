...
- $H_{1}: 0 < Avg_{n}[\text{Discrimination}|\text{Read news} = 1] - Avg_{n}[\text{Discrimination}|\text{Read news} = 0]$
 + .backgrnote[ 
 $H_{0}: 0 = Avg_{n}[\text{Discrimination}|\text{Read news} = 1] - Avg_{n}[\text{Discrimination}|\text{Read news} = 0]$
 ]
...
# But there is a problem: <br> The fundamental problem of causality

.left-column[
.content-box-blue[
.center[
If a person tells us how something affect(ed) them, it is an interesting  personal interpretation.

$\Rightarrow$ *But it is not scientific evidence of a causal effect on them!*
]]
]

.right-column[
```{r, echo = FALSE, out.width='50%', fig.align='center'}
knitr::include_graphics('https://pbs.twimg.com/media/Fcit6yEXwAgZ65w?format=png&name=small')
```
.center[.backgrnote[*Source:* `r Citet(myBib, "llaudet_data_2022")`]]
]
...
- **Nodes** represent variables, **Arrows\edges** represent causal effects `r Citep(myBib, "pearl_causal_2016")`.
  + There must not by a cycle: Follow paths until one re-arrives at the origin.
...
- **Confounder \ Backdoor path bias** = Selection bias.
]
...
because the baseline of the outcome $Y_{0i}$ likely differs between the groups we compare. This is, because certain other variables (i.e. confounders) make us select into / out of $D_{i}$. This is called selection, confounder, or backdoor path bias.
]]]
